...
class FutureEma:
    ...
    def get_candle(self):
        ...
        result = self.exchange.get_historical_data(symbol=self.symbol, start=str(start), end=str(end), time_frame=str(self.time_frame)+"m")
    ...
    def monitor_future(self):
        while True:
            ...
            while now_time.minute % self.time_frame != 0 and self.is_in_trade is None:
                print(now_time.minute % self.time_frame, now_time.minute, f"will wait for 60 sec and  try again")
                time.sleep(60)
                now_time = datetime.now()
            ...
            if ha_close > ema_value and self.is_in_trade is None:
                self.is_in_trade = "BUY"
                self.place_paper_trade(self.is_in_trade)
            elif ha_close < ema_value and self.is_in_trade is None:
                self.is_in_trade = "SELL"
                self.place_paper_trade(self.is_in_trade)
            if ha_close > ema_value and self.is_in_trade == "SELL":
                self.book_profit()
                self.is_in_trade = None
                continue
            elif ha_close < ema_value and self.is_in_trade == "BUY":
                self.book_profit()
                self.is_in_trade = None
                continue
